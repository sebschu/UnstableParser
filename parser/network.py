...
class Network(Configurable):
  ...
  def preload(self):
    '''
       Preloads the model into memory, so that it can be used by parse_online.
    '''
    ...
    # load the model and prep the parse set
    self.preloaded_trainset = Trainset.from_configurable(self, self.vocabs, nlp_model=self.nlp_model)
    with tf.variable_scope(self.name.title()):
      train_tensors = self.preloaded_trainset()
    train_outputs = [train_tensors[train_key] for train_key in self.preloaded_trainset.train_keys]
    ...
    if self.per_process_gpu_memory_fraction == -1:
      ...
      config_proto.gpu_options.per_process_gpu_memory_fraction = self.per_process_gpu_memory_fraction
    self.session_server = tf.train.Server.create_local_server(config=config_proto)
    with tf.Session(self.session_server.target) as sess:
      for var in self.non_save_vars:
        sess.run(var.initializer)
      saver.restore(sess, tf.train.latest_checkpoint(self.save_dir))
      if self.verbose:
        print(ctext('Preloaded model', 'bright_green'))
    return


  def parse_online(self, input_file):
    input_files = [input_file]
    self.add_file_vocabs(input_files)

    with tf.Session(self.session_server.target) as sess:
      parseset = Parseset.from_configurable(self.preloaded_trainset, self.vocabs, parse_files=input_file, nlp_model=self.nlp_model)
      with tf.variable_scope(self.name.title(), reuse=True):
        parse_tensors = parseset(moving_params=self.optimizer)
      parse_outputs = [parse_tensors[parse_key] for parse_key in parseset.parse_keys]
      ...
      for feed_dict, tokens in parseset.iterbatches(shuffle=False):
        ...
        sents.append(tokens)
      if self.verbose:
        print(ctext('Successfully parsed sentences.', 'bright_green'))
      return sents, probs, parseset
